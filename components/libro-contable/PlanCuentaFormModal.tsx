...
const tiposDeCuenta: TipoCuentaContable[] = ['Activo', 'Pasivo', 'Patrimonio', 'Ingreso', 'Costo', 'Gasto'];
...
const PlanCuentaFormModal: React.FC<PlanCuentaFormModalProps> = ({ isOpen, onClose, onSave, cuenta }) => {
    ...
    // This modal opens on top of another modal, so we need a higher z-index
    return (
        <div className="z-[60]">
            <Modal isOpen={isOpen} onClose={onClose} title={cuenta ? 'Editar Cuenta Contable' : 'Nueva Cuenta Contable'}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Input name="codigo" label="Código de Cuenta" value={formData.codigo || ''} onChange={handleChange} required placeholder="Ej: 1.1.01.00001" />
                        <Select
                    ...
                    <Input name="nombre" label="Nombre de la Cuenta" value={formData.nombre || ''} onChange={handleChange} required placeholder="Ej: Efectivo por Depositar" />
